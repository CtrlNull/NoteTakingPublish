...
var CLIENT_ID = '577390244316-a1u6l18ua20c36k18h709p2f28a1lnv8.apps.googleusercontent.com';
var API_KEY = 'AIzaSyDy2zr1hPIZ6b_MjzVp4b40ukzfXssjVmw';
var FOLDER_NAME = "";
var FOLDER_ID = "root";
...
function initClient() {
    gapi.client.init({
        apiKey: API_KEY, //THIS IS OPTIONAL AND WE DONT ACTUALLY dNEED THIS, BUT I INCLUDE THIS AS EXAMPLE
        clientId: CLIENT_ID,
        scope: SCOPES.join(' ')
    }).then(function () {
...
function buildFiles() {
    ...
    if (DRIVE_FILES.length > 0) {
        for (var i = 0; i < DRIVE_FILES.length; i++) {
            DRIVE_FILES[i].isVideo = DRIVE_FILES[i].mimeType.includes('video');
            ...
            if (DRIVE_FILES[i].fileType != "file") {
                fText += "<div class='folder-icon' data-level='" + DRIVE_FILES[i].level + "' data-parent='" + DRIVE_FILES[i].parentID + "' data-size='" + DRIVE_FILES[i].fileSize + "' data-id='" + DRIVE_FILES[i].id + "' title='" + textTitle + "' data-name='" + DRIVE_FILES[i].title + "' data-has-permission='" + DRIVE_FILES[i].hasPermission + "'><div class='image-preview'><img src='/content/images/gvideo/folder.png'/></div></div>";
            } else {
                if (DRIVE_FILES[i].thumbnailLink) {
                    ...
                    fText += "<div class='file-icon'><div class='image-preview'><img src='/content/images/gvideo/" + DRIVE_FILES[i].fileExtension + "-icon.png" + "'/></div></div>";
                }
            }
...
function showLoading() {
    ...
    $("#drive-box-loading").html("<div id='loading-wrapper'><div id='loading'><img src='/content/images/gvideo/loading-bubble.gif'></div></div>");
}
